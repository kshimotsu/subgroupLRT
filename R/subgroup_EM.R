#' @description Estimates parameters of a subgroup model while fixing the value of tau.
#' @export
#' @title subgroupMLE_homo_EM
#' @name subgroupMLE_homo_EM
...
#' # summary(out)
subgroupMLE_homo_EM <- function (y, x, v, m = 2, z = NULL, vcov.method = c("Hessian", "OPG", "none"),
                              ninits = 10, k = 1, tauinit, epsilon = 1e-08, maxit = 2000,
  ...
  if (m == 1) {
    ...
    out0 <- cppSubgroupMLE_homo(b1, y, x, v, ztilde, m, p, maxit,
                               ninits, epsilon, updatetau, emtest)
    ...
    out <- cppSubgroupMLE_homo(b11, y, x, v, ztilde, m, p, maxit=k,
                               ninits=1, epsilon, updatetau, emtest)
