...
#' @export
#' @title subgroupCritBoot_homo_2
#' @name subgroupCritBoot_homo_2
...
subgroupCritBoot_homo_2 <- function (y, x, v, parlist, z = NULL, values = NULL, k = 100, ninits = 100,
  ...
  if (num.cores > 1) {
    ...
    out <- parLapplyLB(cl, 1:nbtsp, function(j) subgroupLRT_homo_2(y=ybset[,j], x = x, v = v,
    ...
    out <- apply(ybset, 2, subgroupLRT_homo_2, x = x, v = v, m = m, z = z,
                              parallel = 0, ninits = ninits, crit.method = "none")
...
}  # end function subgroupCritBoot_homo_2

#' @description Computes the bootstrap critical values of the LRT.
#' @export
#' @title subgroupCritBoot_homo
#' @name subgroupCritBoot_homo
#' @param y n by 1 vector of data for y.
#' @param x n by q vector of data for x.
#' @param parlist parameter estimates as a list containing alpha, mubeta, sigma, and gamma
#' in the form of (alpha = (alpha_1, ..., alpha_m), mubeta = (mubeta_1', ..., mubeta_m'),
...
#' \item{crit}{vector of critical values at the 0.1, 0.05, 0.01 level.}
#' \item{pvals}{p-values corresponding to \code{values}.}
subgroupCritBoot_homo <- function (y, x, v, parlist, z = NULL, values = NULL, ninits = 100,
                            tauinit = tauinit, nbtsp = 199, parallel = 1, cl = NULL) {
  # if (normalregMixtest.env$normalregMix.test.on) # initial values controlled by normalregMix.test.on
  #   set.seed(normalregMixtest.env$normalregMix.test.seed)
  ...
  if (num.cores > 1) {
    ...
    clusterSetRNGStream(cl, 8888577)
    out <- parLapplyLB(cl, 1:nbtsp, function(j) subgroupLRT_homo(y=ybset[,j], x = x, v = v,
                  tauinit = tauinit, m = m, z = z, parallel = 0, ninits = ninits, crit.method = "none"))
    if (newcluster) {
      on.exit(stopCluster(cl))
    } else {
      on.exit(cl)
    ...
    out <- apply(ybset, 2, subgroupLRT_homo, x = x, m = m, z = z, v = v,
                 tauinit = tauinit, ninits = ninits, crit.method = "none", parallel = 0)
  ...
  return(list(crit = crit, pvals = pvals))
}  # end function subgroupCritBoot_homo
